...
import { addProduct } from "@/store/reducers/product-slice.reducer";
...
export function AddProductComponent({
  ...
  setProductDataAdded: (added: boolean) => void;
  addProductModalIsOpen: boolean;
  setAddProductModalIsOpen: (isOpen: boolean) => void;
}) {
  const dispatch = useDispatch();
  const pathname = usePathname();
  const { addProductData } = useSelector((state: any) => state.product);
  ...
  useEffect(() => {
    ...
    if (addProductData.success && allowSetTopBarMsg) {
      setAddProductModalIsOpen(false);
      setProductDataAdded(true);
    ...
    if (!addProductData.success && addProductData.messages) {
      const findMsgByParam = (param: string) => {
        return (
          addProductData.messages.find((obj: any) => obj.param === param)
            ?.msg || ""
        );
      };
      ...
      if (showGeneralErrorMsg) {
        setGeneralErrorMsg(
          addProductData.messages.filter(function (obj: any) {
  ...
  const handleAddProductSubmit = (event: React.FormEvent<HTMLFormElement>) => {
